...
| `DASHBOARD_FREE_ACCOUNT_CAP` | `1000` | The most active Free accounts the service accepts; `0` means no cap. Past it, a new sign-up — email and password, Google, GitHub or One Tap — is refused with *"Free sign-ups are full right now — only paid plans are open…"* (`403`, code `free_signups_full`; OAuth lands on `/login#error=free_signups_full`). Counted as accounts not deleted and not on a paid plan, so deleting an account or moving it to Pro frees a place; an existing account always signs in. Checked before any code is emailed, and again in the same synchronous turn as the account is created. Anything but a whole number **exits**. |
| `DASHBOARD_FREE_WAITLIST_FILE` | `free-signups-refused.jsonl` beside `DB_PATH` | Where a sign-up refused by the cap is recorded, one JSON object per line (`email`, `via`, `at`), each address once. Not a waitlist: nobody is told they are on it and nothing reads it back — it tells the owner who wanted in. Personal data: keep it out of git and backups you share. Beside the database by default so the systemd unit's one writable path covers it. |
